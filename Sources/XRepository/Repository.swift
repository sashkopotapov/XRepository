...
public protocol Repository {
  ...
  func getElements(filteredBy filter: Query<Model>?,
                   sortedBy sortKeyPath: ComparableKeyPath<Model>?,
                   distinctUsing distinctMode: HashableKeyPath<Model>?) -> AnyRandomAccessCollection<Model>
...
public extension Repository {
  ...
  func getElements<T: Comparable, U: Hashable>(filteredBy filter: Query<Model>? = nil, sortedBy sortKeyPath: KeyPath<Model, T>? = nil, distinctUsing distinctKeyPath: KeyPath<Model, U>? = nil) -> AnyRandomAccessCollection<Model> {
  ...
  func getElements<P: Predicate>(filteredByPredicate predicate: @autoclosure () -> P) -> AnyRandomAccessCollection<Model> where P.ResultType == Model {
    let filter = Query(predicate())
    return getElements(filteredBy: filter, sortedBy: nil, distinctUsing: nil)
  }
  
  func getElements<P: Predicate, T: Comparable>(filteredByPredicate predicate: @autoclosure () -> P, sortedBy sortKeyPath: KeyPath<Model, T>) -> AnyRandomAccessCollection<Model> where P.ResultType == Model {
    let filter = Query(predicate())
    let sortMode = ComparableKeyPath(sortKeyPath)
    return getElements(filteredBy: filter, sortedBy: sortMode, distinctUsing: nil)
  }
  
  func getElements<T: Comparable>(sortedBy sortKeyPath: KeyPath<Model, T>) -> AnyRandomAccessCollection<Model> {
    let sortMode = ComparableKeyPath(sortKeyPath)
    return getElements(filteredBy: nil, sortedBy: sortMode, distinctUsing: nil)
  }
  
  func getElements<U: Hashable>(distinctUsing distinctKeyPath: KeyPath<Model, U>) -> AnyRandomAccessCollection<Model> {
  ...
  func getElements<P: Predicate, T: Comparable, U: Hashable>(filteredByPredicate predicate: @autoclosure () -> P, sortedBy sortKeyPath: KeyPath<Model, T>, distinctUsing distinctKeyPath: KeyPath<Model, U>) -> AnyRandomAccessCollection<Model> where P.ResultType == Model {
    let filter = Query(predicate())
    let sortMode = ComparableKeyPath(sortKeyPath)
    let distinctMode = HashableKeyPath(distinctKeyPath)
